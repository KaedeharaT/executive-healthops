...
def test_core_display_mappings_are_chinese():
    ...
    assert provider("apple_health") == "Apple Health"
    ...
    assert risk_level("YELLOW") == "需要关注"
    assert risk_level("RED") == "高优先级"
    assert risk_level("UNKNOWN") == "暂无法判断"
    assert risk_level("GRAY") == "暂无法判断"
