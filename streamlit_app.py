...
def _render_portfolio_landing() -> None:
    """A calm first screen for the isolated portfolio demo, not a dev dashboard."""
    st.markdown(
...
def render_more_workspace() -> None:
    _page_header("更多", "数据接入、专业资料与系统管理。", eyebrow="平台工具")
    ...
    if more is None:
        _section_header("管理工具", "选择一个工具后才加载对应内容。")
        entries = [
    ...
    if more == "数据接入与设备":
        ...
        st.subheader("操作记录")
        members = _members()
        patient = st.selectbox("选择成员", members, format_func=_member_display, key="audit-member")
