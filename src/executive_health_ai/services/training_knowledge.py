"""Original synthetic internal knowledge for Executive HealthOps.
...
HEALTHOPS_INTERNAL_KNOWLEDGE_V1: tuple[InternalKnowledgeSpec, ...] = (
    InternalKnowledgeSpec("report-review", "体检报告审核SOP", "INTERNAL_SOP", (
    ...
    InternalKnowledgeSpec("finding-confirmation", "异常结果确认与入档规范", "INTERNAL_SOP", (
    ...
    InternalKnowledgeSpec("yellow-risk", "Yellow Risk健管处理SOP", "INTERNAL_SOP", (
    ...
    InternalKnowledgeSpec("red-risk", "Red Risk人工运营处置SOP", "INTERNAL_SOP", (
    ...
    InternalKnowledgeSpec("doctor-escalation", "健管到内部医生升级规范", "INTERNAL_SOP", (
    ...
    InternalKnowledgeSpec("health-plan", "健康计划建立与成员确认规范", "INTERNAL_SOP", (
    ...
    InternalKnowledgeSpec("member-task", "成员任务未完成处理规范", "INTERNAL_SOP", (
    ...
    InternalKnowledgeSpec("service-operation", "服务申请、安排与结果回流SOP", "SERVICE_SOP", (
    ...
    InternalKnowledgeSpec("outcome-review", "Outcome阶段复盘与下一步规范", "INTERNAL_SOP", (
    ...
    InternalKnowledgeSpec("data-freshness", "健康数据缺失与陈旧数据处理规范", "INTERNAL_SOP", (
    ...
    InternalKnowledgeSpec("communication", "健管客户沟通基本规范", "COMMUNICATION", (
    ...
    InternalKnowledgeSpec("common-errors", "HealthOps安全边界与常见流程错误", "AI_SAFETY", (
...
def seed_healthops_internal_knowledge(session: Session) -> dict[str, int]:
    """Add or reclassify owned demo knowledge without replacing user content."""
    service = KnowledgeService()
    created_documents = 0
    created_chunks = 0
    for spec in HEALTHOPS_INTERNAL_KNOWLEDGE_V1:
        external_id = f"healthops-internal-v1-{spec.code}"
        legacy_external_id = f"portfolio-training-v1-{spec.code}"
        existing = session.scalar(select(KnowledgeDocument).where(
            KnowledgeDocument.source_external_id.in_((external_id, legacy_external_id)),
        ))
        if existing is not None:
            existing.title = spec.title
            existing.category = spec.category
            existing.source_type = spec.category
            existing.source_name = "Executive HealthOps Internal Demo Knowledge"
            existing.source_provider = "HEALTHOPS_INTERNAL"
            existing.source_external_id = external_id
            existing.license_note = "本项目原创 Internal Demo Knowledge；不代表医院制度或临床指南。"
            existing.attribution = "Executive HealthOps Internal Demo Knowledge · Synthetic / Demo"
            existing.tags = ["Internal Demo Knowledge", "Synthetic", spec.category]
            existing.metadata_json = {
                **(existing.metadata_json or {}),
                "jurisdiction": ["GLOBAL"],
                "audience": ["HEALTH_MANAGER", "DOCTOR", "AI_INTERNAL"],
                "intended_use": ["WORKFLOW_GUIDANCE"],
                "prohibited_use": ["NOT_FOR_DIAGNOSIS", "NOT_FOR_PRESCRIPTION", "NOT_FOR_AUTOMATIC_RISK", "NOT_FOR_EMERGENCY_AUTOMATION"],
                "portfolio_training_sop": False,
            }
            continue
        document = service.create_document(
            ...
            source_name="Executive HealthOps Internal Demo Knowledge",
            source_provider="HEALTHOPS_INTERNAL",
            source_external_id=external_id,
            summary=spec.sections[0][1],
            content_text=spec.content,
            version="v1.0",
            retrieved_at=datetime(2026, 9, 1, tzinfo=timezone.utc),
            license_note="本项目原创 Internal Demo Knowledge；不代表医院制度或临床指南。",
            attribution="Executive HealthOps Internal Demo Knowledge · Synthetic / Demo",
            review_status="PENDING_REVIEW",
            review_due_at=date(2027, 9, 1),
            tags=("Internal Demo Knowledge", "Synthetic", spec.category),
            metadata_json={
                "portfolio_demo": True,
                "synthetic_demo": True,
                "portfolio_training_sop": False,
        ...
        service.approve_document(session, document, "Portfolio Demo Governance", "原创演示资料已按当前产品工作流核对；仅用于 HealthOps 演示。")
        created_documents += 1
...
# Migration and downstream-import compatibility. Current product code must use
# the generic names above; no current UI depends on the old training product.
TrainingKnowledgeSpec = InternalKnowledgeSpec
TRAINING_KNOWLEDGE_V1 = HEALTHOPS_INTERNAL_KNOWLEDGE_V1


def seed_training_knowledge(session: Session) -> dict[str, int]:
    """Deprecated compatibility alias for pre-correction tooling."""
    return seed_healthops_internal_knowledge(session)
